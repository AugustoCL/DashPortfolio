...
dir.create(path.expand("~/.fonts"), showWarnings = FALSE, recursive = TRUE)
file.copy(".fonts/LucidaSansUnic.ttf", path.expand("~/.fonts/"), overwrite = TRUE)
system("fc-cache -f ~/.fonts")

font_ggplot <- "LucidaSansUnic"

end_date_default   <- lubridate::floor_date(Sys.Date(), "month") - 1
start_date_default <- end_date_default - lubridate::years(3)
```
...
fluidRow(
  column(7,
  textInput("stock4", "Ação 4", "PETR4.SA")),
  column(4,
...
fluidRow(
  column(7,
  textInput("stock5", "Ação 5", "ITUB4.SA")),
  column(4,
...
fluidRow(
  column(7,
  dateInput("date_begin", "Data de Início", start_date_default, format = "yyyy-mm-dd"))
)

fluidRow(
  column(7,
  dateInput("date_end", "Data Final", end_date_default, format = "yyyy-mm-dd"))
)
...
prices <- eventReactive(input$go, ignoreNULL = FALSE, {
    ...
    validate(need(length(symbols) > 0, "Informe ao menos um ticker para calcular."))

    prices_long <- tryCatch(
        suppressWarnings(tidyquant::tq_get(symbols, get = "stock.prices",
                          from = input$date_begin, to = input$date_end)),
        error = function(e) NULL
    )
    validate(need(!is.null(prices_long) && nrow(prices_long) > 0,
                  "Não foi possível buscar os dados no Yahoo Finance. Verifique os tickers e as datas."))

    missing <- setdiff(symbols, unique(prices_long$symbol))
    validate(need(length(missing) == 0,
                  paste0("Ticker(s) não encontrado(s) no Yahoo Finance: ",
                         paste(missing, collapse = ", "),
                         ". Corrija ou remova para calcular.")))
...
prices_periodo <- eventReactive(input$go, ignoreNULL = FALSE, {
    ...
    if(input$periodo == "Mensal"){
        data <- prices() %>% 
            to.monthly(indexAt = "endof", OHLC = FALSE)
    } else if(input$periodo == "Semanal"){
        data <- prices() %>% 
            to.weekly(indexAt = "endof", OHLC = FALSE)
    } else {
        data <- prices() %>% 
            to.yearly(indexAt = "endof", OHLC = FALSE)
    }
  prices_periodo <- data
})
```
...
asset_returns_tq_builtin <- eventReactive(input$go, ignoreNULL = FALSE, {
    asset_returns_xts() %>%
      ...
      pivot_longer(-date, names_to = "asset", values_to = "retornos")
})
...
ret_hist <- eventReactive(input$go, ignoreNULL = FALSE, {
  ret_hist <- asset_returns_long() %>% 
    pivot_wider(names_from = asset, values_from = retornos) %>% 
